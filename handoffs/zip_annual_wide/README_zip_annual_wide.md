# Dataset: Wide-Form Zip Code Annual Environmental Health Dataset 

**File Names:**  

**Dynamic Variable Files:**  
`zip_2010.csv` … `zip_2024.csv`  
...
This dataset provides annual summaries of environmental and climate health variables for all U.S. zip codes from 2010 to 2024, stored in a **wide format**. Each row corresponds to a single zip code and year, with environmental variables represented as separate columns.

## Usage

The wide-form dataset is optimized for regression modeling, machine learning, and other analyses that require multiple covariates in the same row. It simplifies direct modeling by providing environmental exposures and contextual variables as features aligned by zip code and year.

## Structure

## Structure 

Each file (`zip_<year>.csv`) is a wide-form dataset where:

- Each row represents a single zip code (identified by `geoid`).
- Each column represents an environmental or climate variable for that year.
- The `zip_static.csv` file contains variables that do not vary by year.
- See Covariate Descriptions below for a comprehensive list of variables included.
