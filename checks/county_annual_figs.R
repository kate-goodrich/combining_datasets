...
# ---- Generic normal map function (illusion-safe) ----
plot_normal_map <- function(
    ...
    direction = 1,
    bbox = c(-140, -60, 18, 72),
    limits = NULL, # <- optionally fix scale limits
    out_file = NULL,
    title = NULL,
    border_col = "#9b8ea7", # <- constant subtle border
    border_size = 0.08 # <- small stroke to kill the seam illusion
) {
    level <- match.arg(level)
    geom_layer <- if (level == "county") "counties_500k" else "tracts_500k"
    ...
    if (is.null(limits)) {
        if (identical(var, "soltotal")) {
            ...
            # robust limits for other vars (avoid outlier-driven dark maps)
    ...
    p <- ggplot(plot_df) +
        ...
        theme_minimal() +
        theme(
            panel.grid.major = element_blank(), # remove graticule lines
...
plot_normal_map(
    ...
    palette = "inferno",
    limits = c(0, 20), # keep the scale stable
